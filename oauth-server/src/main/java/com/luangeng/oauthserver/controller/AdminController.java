...
public class AdminController {
	...
	@RequestMapping("/token/{client}/{user}")
	@ResponseBody
	public List<OAuth2AccessToken> listTokensForUser(@PathVariable String client, @PathVariable String user, Principal principal) {
		return enhance(tokenStore.findTokensByClientIdAndUserName(client, user));
	}

	@RequestMapping(value = "/token/{user}/{token}", method = RequestMethod.DELETE)
	...
	@RequestMapping("/token/{client}")
	public List<OAuth2AccessToken> listTokensForClient(@PathVariable String client) throws Exception {
